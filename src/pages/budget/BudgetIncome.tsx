...
export function BudgetIncome() {
    ...
    const { records, loading, error, totalThisMonth, refetch } = useBudgetIncome();
    const { incomeCategories } = useBudgetCategories();
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const category = incomeCategories.find(c => c.id === form.category_id);
    ...
    return (
        <div>
            <PageHeader
                ...
                subtitle="Track all money coming into your household"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                {[
                    { id: 'month', label: isFiltered ? 'Filtered Total' : 'This Month', value: isFiltered ? filteredTotal : totalThisMonth },
                    { id: 'count', label: 'Records', value: filtered.length },
                ].map(s => (
                    <div key={s.id} className="rounded-xl p-4" style={{ background: 'var(--ff-surface)', border: '1px solid var(--ff-border)' }}>
                        ...
                        <p className="text-xl font-bold" style={{ color: '#22c55e' }}>
                            {typeof s.value === 'number' && s.id !== 'count' ? `ZMW ${fmt(s.value)}` : s.value}
                        </p>
                    </div>
                ))}
            </div>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-36 rounded-xl" style={{ background: 'var(--ff-surface)', border: '1px solid var(--ff-border)' }}>
                    <TrendingUp size={36} style={{ color: 'var(--ff-border)' }} className="mb-3" />
                    <p className="text-sm" style={{ color: 'var(--ff-text-muted)' }}>No income records yet. Add your first entry above.</p>
                </div>
            ) : (
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-0.5">
                                    <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: '#22c55e20', color: '#22c55e' }}>
                                        {r.metadata?.category_name ?? 'Income'}
                                    </span>
                                ...
                                <p className="text-sm font-medium" style={{ color: 'var(--ff-text-primary)' }}>
                                    {r.description || r.metadata?.category_name || 'Income'}
                                </p>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }}>
                    <div className="w-full max-w-md rounded-2xl p-6" style={{ background: 'var(--ff-surface)', border: '1px solid var(--ff-border)' }}>
                        <h2 className="text-lg font-semibold mb-5" style={{ color: 'var(--ff-text-primary)' }}>Add Income</h2>
                        <form onSubmit={handleSave} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" value={form.description}
                                    onChange={e => setForm(f => ({ ...f, description: e.target.value }))} style={inputStyle} placeholder="e.g. Monthly salary" />
                            </div>
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setShowModal(false)}
                                    className="flex-1 py-2 rounded-lg text-sm" style={{ border: '1px solid var(--ff-border)', color: 'var(--ff-text-muted)', background: 'transparent' }}>
                                    Cancel
                                </button>
                                <button type="submit" disabled={saving}
                                    className="flex-1 py-2 rounded-lg text-sm font-medium text-white" style={{ background: 'var(--ff-accent)', opacity: saving ? 0.7 : 1 }}>
                                    {saving ? 'Saving…' : 'Save'}
                                </button>
                            </div>
                        </form>
