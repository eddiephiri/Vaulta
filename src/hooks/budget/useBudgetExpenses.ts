...
import type { BudgetExpenseRecord } from '../../types';
...
interface UseBudgetExpensesReturn {
    records: BudgetExpenseRecord[];
    loading: boolean;
    error: string | null;
    totalToday: number;
...
export function useBudgetExpenses(): UseBudgetExpensesReturn {
    const [records, setRecords] = useState<BudgetExpenseRecord[]>([]);
    ...
    const fetchRecords = useCallback(async () => {
        ...
        if (supaErr) {
            ...
            setRecords((data ?? []) as BudgetExpenseRecord[]);
        }
        setLoading(false);
    }, []);
